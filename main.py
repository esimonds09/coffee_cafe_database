...
def add_cafe():
    ...
    if form.validate_on_submit():
        print("passed")
        data_list = [form.cafe.data, form.location.data, form.open.data, form.close.data, form.coffee.data, form.wifi.data, form.power.data]
        with open('cafe-data.csv', "a", newline="", encoding="UTF-8") as csv_file:
            # for item in data_list:
                # csv_file.write(f"{item}, ")
            csv_file.write("\n")
            csv_file.writelines(",".join(data_list))
